...
void rebondir(Ball *ball, int plateau[LIGNES][COLONNES], Snoopy *snoopy, PositionInitiale positionInitialeBalle, PositionInitiale positionInitialeSnoopy) {
    ...
    if (plateau[suivantY][suivantX] == 0xC1){  // Bord supérieur
        ball->direction_y = -ball->direction_y;
    }
    else if (plateau[suivantY][suivantX] == 0xC2){  // Bord inférieur
        ball->direction_y = -ball->direction_y;
    }
    else if (plateau[suivantY][suivantX] == 0xC3){  // Bord droit
        ball->direction_x = -ball->direction_x;
    }
    else if (plateau[suivantY][suivantX] == 0xB4){  // Bord gauche
        ball->direction_x = -ball->direction_x;
    }
    if (suivantX == 0 && suivantY == 0) { // Coin supérieur gauche
        ball->direction_x = -ball->direction_x;
        ball->direction_y = -ball->direction_y;
    }
    else if (suivantX == COLONNES - 1 && suivantY == 0) { // Coin supérieur droit
        ball->direction_x = -ball->direction_x;
        ball->direction_y = -ball->direction_y;
    }
    else if (suivantX == 0 && suivantY == LIGNES - 1) { // Coin inférieur gauche
        ball->direction_x = -ball->direction_x;
        ball->direction_y = -ball->direction_y;
    }
    else if (suivantX == COLONNES - 1 && suivantY == LIGNES - 1) { // Coin inférieur droit
        ball->direction_x = -ball->direction_x;
        ball->direction_y = -ball->direction_y;
    }
}
...
void collecteOiseau(int plateau[LIGNES][COLONNES], Snoopy *snoopy) {
    ...
    GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi);// On obtient des informations sur l'écran de la console
        ...
        if (kbhit()){
            ...
            switch (key) {
                case 's':
                    ...
                    break;
                case 'z':
                    if (yflecheNiveau -2 >=yflecheNiveauMin)
                    {yflecheNiveau -= 2;}
                    break;
                case 'p':
                    break;
